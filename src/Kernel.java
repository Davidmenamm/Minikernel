...
public class Kernel {
    ...
    /** Static fields of contained classes. */
    public static DiskQueue scheduler = null;
    ...
    */
    ...
    public static final int INTERRUPT_CACHE = 3;
    ...
    // Cases for read or write operations
    public static final int CASE_READ= 0;
    public static final int CASE_WRITE = 1;
    ...
    */
        ...
        try {
            ...
            case INTERRUPT_USER:
                ...
                case SYSCALL_GET_BLOCK_COUNT:
                    return doGetBlockCount();

                case SYSCALL_GET_BLOCK_SIZE:
                    return doGetBlockSize();

                case SYSCALL_READ_DISK_BLOCK:
                    return doReadWriteCache( i2, a, CASE_READ);

                case SYSCALL_WRITE_DISK_BLOCK:
                    return doReadWriteCache( i2, a, CASE_WRITE);

                default:
                    return ERROR_BAD_ARGUMENT;
                }

            case INTERRUPT_CACHE:
                break;

            case INTERRUPT_DISK:
                scheduler.updateCurrentThread();
                break;
    ...
    private static void doPowerOn(int i1, Object o1, Object o2) {
        ...
        // Create an instance of DiskQueue
        DiskQueue schedulerQueue = new DiskQueue(disk);
        scheduler = schedulerQueue;

        cache = new DiskCache(disk, schedulerQueue, i1);
    ...
    /** Returns current time of the system in miliseconds
     * @param time receives a one element long array to store the time
     * @return the return code returned by the program.
     */
    private static int doGetTime( long[] time ){
     ...
     * @return the return code returned by the program.
     */
    private static int doGetBlockCount(){

        int nBlocks = scheduler.getBlockCount();
        return nBlocks;
    }
    ...
    private static int doGetBlockSize(){
        ...
        int bSize = scheduler.getBlockSize();
        return bSize;
    }

    /** Calls the disk queue for a read or write operation
     * gives as arguments the block number and buffer to write or read to disk
     * @return the return code returned by the program.
     */
    private static int doReadWriteCache( int blockNum, byte[] data, int operationType){

        int rc = cache.readWriteCache(blockNum, data, operationType);
