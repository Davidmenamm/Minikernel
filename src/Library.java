...
public class Library {
     ...
     * @return the current system time in a long data type
     */
     ...
     * Get number of blocks on the disk
     * @return number of blocks as an int or error code
     */
    public static int getDiskBlockCount() {

        int rc = Kernel.interrupt(Kernel.INTERRUPT_USER,
                Kernel.SYSCALL_GET_BLOCK_COUNT,
                0, null, null, null );
     ...
     * Get size of each block in the disk
     * @return block size as an int or error code
     */
    public static int getDiskBlockSize() {

        int rc = Kernel.interrupt(Kernel.INTERRUPT_USER,
                Kernel.SYSCALL_GET_BLOCK_SIZE,
                0, null, null, null );
    ...
    /** Performs SYSCALL_GET_READ_DISK_BLOCK
     * Reads correponding block on disk
     * Receives number of block to read
     * Receives a buffer to fill with the answer
     * @return succesful 0, or error code
     */
    public static int readDiskBlock( int bNum, byte[] data ){

        int rc = Kernel.interrupt( Kernel.INTERRUPT_USER,
                Kernel.SYSCALL_READ_DISK_BLOCK,
                bNum, null, null, data);

        return rc;
    }

    /** Performs SYSCALL_GET_WRITE_DISK_BLOCK
     * Writes correponding block on disk
     * Receives number of block to write
     * Receives a buffer to write on disk
     * @return succesful 0, or error code
     */
    public static int writeDiskBlock( int bNum, byte[] data ){

        int rc = Kernel.interrupt( Kernel.INTERRUPT_USER,
                Kernel.SYSCALL_WRITE_DISK_BLOCK,
                bNum, null, null, data);
